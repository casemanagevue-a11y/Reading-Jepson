...
exports.exchangeGoogleAuthCode = exports.importStudentsFromGoogleClassroom = exports.getGoogleClassroomCourses = exports.submitQuizAttempt = exports.getQuizPublic = exports.generateQuiz = exports.claimStudentAccount = exports.createStudent = void 0;
 ...
 */
async function verifyTeacher(context) {
    if (!context.auth) {
 ...
 */
async function verifyStudent(context) {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const role = await getUserRole(context.auth.uid);
 ...
 */
function calculateNextDueAt(isCorrect, currentStreak) {
    const now = firestore_1.Timestamp.now();
    const nowMillis = now.toMillis();
    if (!isCorrect) {
        // Incorrect: due in 1 day
        return firestore_1.Timestamp.fromMillis(nowMillis + 24 * 60 * 60 * 1000);
    }
    ...
    return firestore_1.Timestamp.fromMillis(nowMillis + daysAhead * 24 * 60 * 60 * 1000);
}
/**
 * Update mastery status based on streak
 */
function updateMasteryStatus(currentStatus, correctStreak) {
...
exports.createStudent = functions.https.onCall(async (data, context) => {
    ...
    if (!studentEmail || !displayName) {
        throw new functions.https.HttpsError('invalid-argument', 'studentEmail and displayName are required');
    }
    ...
    if (!existingStudent.empty) {
        throw new functions.https.HttpsError('already-exists', 'Student with this email already exists');
    }
 ...
 * Student only
 */
exports.claimStudentAccount = functions.https.onCall(async (data, context) => {
    const studentUid = await verifyStudent(context);
    // Get student's email from auth
    const user = await auth.getUser(studentUid);
    const studentEmail = user.email;
    if (!studentEmail) {
        throw new functions.https.HttpsError('invalid-argument', 'Student email not found in auth');
    }
    ...
    if (studentQuery.empty) {
        throw new functions.https.HttpsError('not-found', 'No unclaimed student account found for this email');
    }
...
exports.generateQuiz = functions.https.onCall(async (data, context) => {
    ...
    if (!studentUid || !weekId || !mode || !numQuestions) {
        throw new functions.https.HttpsError('invalid-argument', 'studentUid, weekId, mode, and numQuestions are required');
    }
    if (mode !== 'daily' && mode !== 'friday') {
        throw new functions.https.HttpsError('invalid-argument', 'mode must be "daily" or "friday"');
    }
    ...
    const studentQuery = await db.collection('students')
        ...
        .get();
    if (studentQuery.empty) {
        throw new functions.https.HttpsError('permission-denied', 'Student not found or not managed by this teacher');
    }
    // Get week document
    ...
    if (mode === 'daily') {
        ...
        const spiralCount = Math.min(Math.ceil(numQuestions * 0.4), dueWords.length, numQuestions - selectedWords.length);
    ...
    const questions = [];
    const correctMap = {};
    const wordIdsByQuestion = {};
    // Helper to get distractors
    const getDistractors = (correctAnswer, allOptions, count = 3) => {
    ...
    for (const affix of selectedAffixes) {
        ...
        if (questionType === 'affixToMeaning') {
            ...
            const wordWithAffix = currentWeekWords.find(w => w.word.toLowerCase().includes(affix.affix.toLowerCase()));
            if (wordWithAffix) {
    ...
    const assignedAt = firestore_1.Timestamp.now();
    const dueAt = firestore_1.Timestamp.fromMillis(assignedAt.toMillis() + (mode === 'daily' ? 24 * 60 * 60 * 1000 : 3 * 24 * 60 * 60 * 1000));
 ...
 */
exports.getQuizPublic = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { quizId } = data;
...
exports.submitQuizAttempt = functions.https.onCall(async (data, context) => {
    ...
    if (!quizId || !responses || !Array.isArray(responses)) {
        throw new functions.https.HttpsError('invalid-argument', 'quizId and responses array are required');
    }
    // Get public quiz
    ...
    const correctMap = quizPrivate.correctMap;
    const wordIdsByQuestion = quizPrivate.wordIdsByQuestion;
    // Score responses
    ...
    for (const response of scoredResponses) {
        ...
        if (!wordId)
            continue; // Skip if not a word question
 ...
 */
exports.getGoogleClassroomCourses = functions.https.onCall(async (data, context) => {
 ...
 */
exports.importStudentsFromGoogleClassroom = functions.https.onCall(async (data, context) => {
    const teacherUid = await verifyTeacher(context);
