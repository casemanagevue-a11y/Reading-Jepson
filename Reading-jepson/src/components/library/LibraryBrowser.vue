<template>
  <div class="library-browser">
    ...
    <div v-else-if="contentType === 'vocab'" class="items-grid vocab-grid">
      <div 
        v-for="item in displayedVocabItems" 
        :key="item.id"
    ...
    <div v-else-if="contentType === 'affix'" class="items-grid affix-grid">
      <div 
        v-for="item in displayedAffixItems" 
        :key="item.id"
    ...
    <div v-else-if="contentType === 'passage'" class="items-list passage-list">
      <div 
        v-for="item in displayedPassageItems" 
        :key="item.id"
    ...
    <div v-if="viewingPassage" class="modal-overlay" @click="viewingPassage = null">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          ...
          <button @click="viewingPassage = null" class="btn-close">×</button>
        </div>
...
const displayedVocabItems = computed<VocabLibraryWithId[]>(() => {
  return contentType.value === 'vocab' ? vocabItems.value : []
})

const displayedAffixItems = computed<AffixLibraryWithId[]>(() => {
  return contentType.value === 'affix' ? affixItems.value : []
})

const displayedPassageItems = computed<PassageLibraryWithId[]>(() => {
  return contentType.value === 'passage' ? passageItems.value : []
})
...
async function loadLibraryItems() {
  if (!user.value) {
    console.log('[LibraryBrowser] Waiting for user authentication...')
    // Wait a bit for auth to complete
    setTimeout(() => {
      if (user.value) loadLibraryItems()
    }, 500)
    return
  }
  
  console.log('[LibraryBrowser] Loading library items, type:', contentType.value)
  console.log('[LibraryBrowser] User UID:', user.value.uid)
  ...
  try {
    ...
    console.log('[LibraryBrowser] Filter params:', filterParams)

    if (contentType.value === 'vocab') {
      const results = await libraryServices.queryVocabLibrary(filterParams)
      console.log('[LibraryBrowser] Vocab results:', results.length)
      vocabItems.value = results
    } else if (contentType.value === 'affix') {
      const results = await libraryServices.queryAffixLibrary(filterParams)
      console.log('[LibraryBrowser] Affix results:', results.length)
      affixItems.value = results
    } else {
      const results = await libraryServices.queryPassageLibrary(filterParams)
      console.log('[LibraryBrowser] Passage results:', results.length)
      passageItems.value = results
    }
  } catch (error) {
    console.error('[LibraryBrowser] Error loading library items:', error)
