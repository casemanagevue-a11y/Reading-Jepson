    ...
    <div class="container">
      ...
      <div v-else>
        ...
        <div class="planner-controls">
          ...
          <p class="edit-note">
            {{ editMode ? 'Drag assignments to move them or click × to delete' : 'Click Edit Mode to reorganize assignments' }}
          </p>
        </div>
        ...
        <div class="calendar-section">
          <div class="calendar-controls">
            <button @click="previousMonth" class="btn btn-secondary">← Previous Month</button>
            <h2>{{ currentMonthName }} {{ currentYear }}</h2>
            <button @click="nextMonth" class="btn btn-secondary">Next Month →</button>
          </div>
          ...
          <div class="calendar-grid">
            ...
            >
              ...
              <div 
                ...
                :draggable="editMode"
                @dragstart="editMode ? handleAssignmentDragStart($event, day.assignment) : null"
              >
                ...
                <div class="assignment-actions">
                  ...
                  <button v-if="editMode" @click.stop="removeAssignment(day.assignment.id)" class="remove-btn">×</button>
                </div>
              </div>
            </div>
          </div>
...
const draggedAssignment = ref<(WeekAssignmentDocument & { id: string }) | null>(null)
...
const daysOffSet = computed(() => {
  ...
  calendarEvents.value.forEach(event => {
    if (event.eventType === 'dayOff' || event.eventType === 'holiday' || 
      ...
      set.add(dateKey)
      console.log('[VisualPlanner] Added day off:', dateKey)
  ...
  console.log('[VisualPlanner] Total days off:', set.size, Array.from(set))
  return set
})
...
const handleAssignmentDragStart = (event: DragEvent, assignment: WeekAssignmentDocument & { id: string }) => {
...
const handleDragOver = (event: DragEvent, day: any) => {
  if (!day.isSchoolDay || (!draggedTemplate.value && !draggedAssignment.value)) return
...
const handleDrop = async (event: DragEvent, day: any) => {
  ...
  if (!day.isSchoolDay || (!draggedTemplate.value && !draggedAssignment.value) || !user.value) return
  
  try {
    // Handle moving existing assignment
    if (draggedAssignment.value && editMode.value) {
      const assignment = draggedAssignment.value
      const newStartDate = day.date
      
      // Calculate new end date based on template length
      const template = templates.value.find(t => t.id === assignment.weekTemplateId)
      if (!template) {
        error.value = 'Template not found'
        return
      }
      
      const newEndDate = calculateEndDate(newStartDate, template.weekLength)
      const quarter = calculateQuarterFromDate(newStartDate)
      
      // Delete old assignment and create new one at new date (like working version does)
      await deleteWeekAssignment(assignment.id)
      await createWeekAssignment({
        ...
        actualDays: [1, 2, 3, 4, 5],
        status: assignment.status
      })
      
      success.value = `Moved assignment to ${newStartDate.toLocaleDateString()}`
    ...
    // Handle dropping new template
    if (!draggedTemplate.value) return
